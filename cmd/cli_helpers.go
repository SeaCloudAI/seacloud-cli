...
func validateOutputFormat(flagName, value string, allowed ...string) error {
	...
	return cliParamError(flagName, fmt.Sprintf("unsupported value %q; allowed values are: %s", value, strings.Join(allowed, ", ")), "Use --output json for structured output, or omit the flag for table output.")
}
...
func cliParamError(param, issue, next string) error {
...
func cliMissingParam(param, next string) error {
	return &clierrors.CLIError{
