...
Output defaults to a compact table for humans. Use --output json for Agent-safe
structured output. List and log commands expose --limit plus cursor or token flags
so Agents can keep responses small.
...
Use --output json when another tool or Agent needs the full page object,
including items, hasNext, and nextToken.`,
...
deleted.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if IsDryRun() {
			...
			if len(args) == 0 {
				return aiMissingParam("<sandbox-id...>", "Pass one or more IDs, or run: seacloud --dry-run sandbox kill --all --state running")
			}
		...
		if len(ids) == 0 {
			return aiMissingParam("<sandbox-id...>", "Pass one or more IDs, or use --all with filters such as --state running.")
		}
		for _, id := range ids {
...
entries.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(body) == 0 {
			return aiMissingParam("--allow-public-traffic/--allow-internet-access/--allow-out/--deny-out", "Add at least one policy flag, for example: seacloud --dry-run sandbox network update sb_123 --allow-internet-access=false")
		}
		if IsDryRun() {
...
with --volume-mount name:/path. Create/delete support --dry-run so Agents can
...
the response bounded for Agents.`,
	Args: cobra.MaximumNArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
  ...
  --dead-letter-enabled and --dead-letter-url configure exhausted delivery flow.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(webhookCreateOpts.name) == "" {
			return aiMissingParam("--name", "Add --name, for example: seacloud --dry-run sandbox webhook create --name lifecycle --url https://example.com/hook --secret $WEBHOOK_SECRET")
		}
		if strings.TrimSpace(webhookCreateOpts.url) == "" {
			return aiMissingParam("--url", "Add an HTTPS callback URL with --url.")
		}
		if strings.TrimSpace(webhookCreateOpts.secret) == "" {
			return aiMissingParam("--secret", "Add --secret or inject a secret from your secret manager.")
		}
...
func parseTriBool(raw, name string) (bool, bool, error) {
	...
	if err != nil {
		return false, false, aiParamError("--"+name, "must be true or false", "Use true or false, for example: --"+name+"=false")
	}
