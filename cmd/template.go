...
Help text is designed to be used as Agent documentation: each command documents
defaults, required flags, output shape, and pagination controls. Use --format json
or --output json for structured output. Use --dry-run before build, delete, init,
migrate, or tag mutation commands to preview the exact request or file writes.`,
...
func buildTemplateDefinition() (*sandboxgo.Template, string, error) {
	...
	default:
		...
		return nil, "", aiMissingParam("--dockerfile/--image/--from-template", "Pass a source flag or add ./Dockerfile, for example: seacloud --dry-run template build demo --image python:3.13")
	}
}
...
func writeTemplateProject(dir, language, name string, force, migrate bool) error {
	...
	case "py", "python":
		...
		return writeTemplateFiles(dir, force, files)
	default:
		return aiParamError("--language", "must be typescript or python", "Use: seacloud template init --language typescript or seacloud template init --language python")
	}
}
