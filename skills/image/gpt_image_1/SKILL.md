...
name: gpt_image_1
description: >-
  Use the `gpt_image_1` model via the SeaCloud CLI for broad OpenAI image generation, prompt-led visual ideation, and flexible image workflow coverage.
  Make sure to use this skill for any general-purpose image task, any quick visual exploration, any prompt-driven concept generation, or any request that benefits from broad OpenAI-style image output without immediately optimizing for the most premium final polish.
  Prefer this skill over heavier or more premium image models when breadth, familiarity, and fast iteration matter more than maximum finish.
---

# `gpt_image_1` - GPT Image 1

**Provider:** OpenAI | **CLI Model ID:** `gpt_image_1` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec gpt_image_1 --output json
```

Build the `seacloud run gpt_image_1 ...` command from the current JSON spec every time.
...
If `seacloud models spec gpt_image_1 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec gpt_image_1 --output json`
