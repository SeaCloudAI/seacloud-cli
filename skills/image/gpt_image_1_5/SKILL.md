...
name: gpt_image_1_5
description: >-
  Use the `gpt_image_1_5` model via the SeaCloud CLI for stronger OpenAI image generation, cleaner composition control, and more premium prompt-driven visuals.
...
```bash
seacloud models spec gpt_image_1_5 --output json
```

Build the `seacloud run gpt_image_1_5 ...` command from the current JSON spec every time.
...
If `seacloud models spec gpt_image_1_5 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec gpt_image_1_5 --output json`
