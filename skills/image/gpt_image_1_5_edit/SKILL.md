---
name: gpt_image_1_5_edit
description: >-
  Use the `gpt_image_1_5_edit` model via the SeaCloud CLI for refined single-image editing, stronger text replacement, and cleaner presentation-ready revisions.
  Make sure to use this skill whenever the user already has one source image and wants more polished edits, stronger instruction following, or higher-confidence commercial retouching.
  Prefer this skill over `gpt_image_1_edit` when finish and edit quality matter more than raw iteration speed.
---

# `gpt_image_1_5_edit` - GPT Image 1.5 Edit

**Provider:** OpenAI | **CLI Model ID:** `gpt_image_1_5_edit` | **Inputs:** Text | Image | **Outputs:** Text | Image
...
```bash
seacloud models spec gpt_image_1_5_edit --output json
```

Build the `seacloud run gpt_image_1_5_edit ...` command from the current JSON spec every time.
...
If `seacloud models spec gpt_image_1_5_edit --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec gpt_image_1_5_edit --output json`
