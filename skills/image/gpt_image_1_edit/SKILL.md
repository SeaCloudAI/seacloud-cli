---
name: gpt_image_1_edit
description: >-
  Use the `gpt_image_1_edit` model via the SeaCloud CLI for broad single-image editing, controlled revisions, text replacement, and practical visual cleanup.
  Make sure to use this skill whenever the user already has one source image and wants fast edits, structured changes, or iterative revision passes without immediately optimizing for the highest-end finish.
  Prefer this skill over heavier or more premium edit models when speed, accessibility, and broad edit coverage matter more than polish.
---

# `gpt_image_1_edit` - GPT Image 1Edit

**Provider:** OpenAI | **CLI Model ID:** `gpt_image_1_edit` | **Inputs:** Text | Image | **Outputs:** Text | Image
...
```bash
seacloud models spec gpt_image_1_edit --output json
```

Build the `seacloud run gpt_image_1_edit ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload is centered on flat top-level fields such as `image`, `prompt`, `mask`, `n`.
...
4. For this model, verify the main fields first: `image`, `prompt`, `mask`, `n`, `size`, `quality`.
5. Required child paths to keep intact: `image`, `prompt`.
...
If `seacloud models spec gpt_image_1_edit --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec gpt_image_1_edit --output json`
