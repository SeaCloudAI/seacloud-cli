...
name: kling_image_o1
description: >-
  Use the `kirin_image_omni` model via the SeaCloud CLI for premium multi-input Kling image generation and editing with stronger detail and refined visual finish.
  Make sure to use this skill whenever the user wants high-end image work that may combine prompt input with one or more reference images rather than a simple single-input flow.
  Prefer this skill over lighter Kling image models when premium polish matters more than speed.
---

# `kirin_image_omni` - Kling Image O1

**Provider:** Kling | **CLI Model ID:** `kirin_image_omni` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec kirin_image_omni --output json
```

Build the `seacloud run kirin_image_omni ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_image_omni --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_image_omni --output json`
