...
name: kling_v3_image
description: >-
  Use the `kirin_v3_image` model via the SeaCloud CLI for general image generation, editing, subject-consistent reference work, and wider multi-image creative control.
  Make sure to use this skill for any request that mixes text generation with image references, image editing, subject consistency, or broader Kling image workflow coverage.
  Prefer this skill when the user wants one image model that can handle both fresh generation and richer reference-driven manipulation.
---

# `kirin_v3_image` - Kling V3 Image

**Provider:** Kling | **CLI Model ID:** `kirin_v3_image` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec kirin_v3_image --output json
```

Build the `seacloud run kirin_v3_image ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `element_list`, `watermark_info`.
...
4. For this model, verify the main fields first: `prompt`, `negative_prompt`, `image`, `image_reference`, `image_fidelity`, `human_fidelity`.
5. Required child paths to keep intact: `prompt`, `element_list.element_id`.
...
If `seacloud models spec kirin_v3_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v3_image --output json`
