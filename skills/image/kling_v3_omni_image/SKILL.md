...
description: >-
  Use the `kirin_v3_omni_image` model via the SeaCloud CLI for high-end V3-era Kling image generation with multi-input flexibility and premium finish.
...
```bash
seacloud models spec kirin_v3_omni_image --output json
```

Build the `seacloud run kirin_v3_omni_image ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v3_omni_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v3_omni_image --output json`
