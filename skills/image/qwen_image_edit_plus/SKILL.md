...
description: >-
  Use the `qwen_image_edit_plus` model via the SeaCloud CLI for higher-fidelity image editing, text replacement, controlled retouching, and multi-image reference-driven visual changes.
...
```bash
seacloud models spec qwen_image_edit_plus --output json
```

Build the `seacloud run qwen_image_edit_plus ...` command from the current JSON spec every time.
...
If `seacloud models spec qwen_image_edit_plus --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec qwen_image_edit_plus --output json`
