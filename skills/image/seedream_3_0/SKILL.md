...
name: seedream_3_0
description: >-
  Use the `spark_dream_3_0` model via the SeaCloud CLI for stable Seedream text-to-image generation with reliable prompt alignment and lightweight creative drafting.
  Make sure to use this skill for any straightforward text-to-image request where the user wants a dependable first pass, scalable drafting, or practical visual output without asking for premium finishing.
  Prefer this skill over newer Seedream variants when consistency and simplicity matter more than richer detail or stronger polish.
---

# `spark_dream_3_0` - Seedream 3.0

**Provider:** Volces | **CLI Model ID:** `spark_dream_3_0` | **Inputs:** Text | **Outputs:** Image
...
```bash
seacloud models spec spark_dream_3_0 --output json
```

Build the `seacloud run spark_dream_3_0 ...` command from the current JSON spec every time.
...
If `seacloud models spec spark_dream_3_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec spark_dream_3_0 --output json`
