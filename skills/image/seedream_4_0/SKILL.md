...
name: seedream_4_0
description: >-
  Use the `spark_dream_4_0` model via the SeaCloud CLI for balanced Seedream image generation with better lighting, detail, and prompt understanding than the lightest baseline.
  Make sure to use this skill for any request that wants a middle ground between fast drafting and premium rendering, especially when the user wants a stronger second-step option without jumping all the way to the highest-end Seedream variants.
  Prefer this skill when the request sounds like balanced quality rather than either cheapest iteration or maximum polish.
---

# `spark_dream_4_0` - Seedream 4.0

**Provider:** Volces | **CLI Model ID:** `spark_dream_4_0` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec spark_dream_4_0 --output json
```

Build the `seacloud run spark_dream_4_0 ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `sequential_image_generation_options`, `optimize_prompt_options`, `tools`.
...
4. For this model, verify the main fields first: `prompt`, `image`, `size`, `n`, `seed`, `response_format`.
...
If `seacloud models spec spark_dream_4_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec spark_dream_4_0 --output json`
