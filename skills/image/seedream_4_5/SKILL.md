...
name: seedream_4_5
description: >-
  Use the `spark_dream_4_5` model via the SeaCloud CLI for premium Seedream image generation with stronger consistency, richer detail, and higher creative control.
  Make sure to use this skill for any image request that sounds premium, brand-facing, presentation-ready, or clearly above baseline quality, especially when the user wants more control and polish without necessarily demanding the newest flagship variant.
  Prefer this skill over lighter Seedream models when visual quality and controlled refinement matter more than speed.
---

# `spark_dream_4_5` - Seedream 4.5

**Provider:** Volces | **CLI Model ID:** `spark_dream_4_5` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec spark_dream_4_5 --output json
```

Build the `seacloud run spark_dream_4_5 ...` command from the current JSON spec every time.
...
If `seacloud models spec spark_dream_4_5 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec spark_dream_4_5 --output json`
