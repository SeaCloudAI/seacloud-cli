...
name: seedream_5_0
description: >-
  Use the `spark_dream_5_0` model via the SeaCloud CLI for flagship Seedream image generation with the highest-end detail, polish, and premium visual finish in this family.
  Make sure to use this skill for any request that sounds final-deliverable, hero-quality, premium campaign-grade, or like the user wants the strongest Seedream rendering available rather than a balanced or lightweight option.
  Prefer this skill over `spark_dream_4_5` when the request clearly asks for the top-end result, not just a premium one.
---

# `spark_dream_5_0` - Seedream 5.0

**Provider:** Volces | **CLI Model ID:** `spark_dream_5_0` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec spark_dream_5_0 --output json
```

Build the `seacloud run spark_dream_5_0 ...` command from the current JSON spec every time.
...
If `seacloud models spec spark_dream_5_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec spark_dream_5_0 --output json`
