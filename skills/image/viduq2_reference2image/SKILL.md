---
name: viduq2_reference2image
description: >-
  Use the `viduq2_reference2image` model via the SeaCloud CLI for reference-guided image generation with stronger style and subject consistency.
  Make sure to use this skill whenever the user wants to create a new image from one or more references while preserving identity, style, or visual consistency.
  Prefer this skill over broad general image models when reference alignment matters more than open-ended exploration.
---

# `viduq2_reference2image` - Vidu Q2 - Reference to Image

**Provider:** Vidu | **CLI Model ID:** `viduq2_reference2image` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec viduq2_reference2image --output json
```

Build the `seacloud run viduq2_reference2image ...` command from the current JSON spec every time.
...
4. For this model, verify the main fields first: `images`, `prompt`, `seed`, `aspect_ratio`, `resolution`, `payload`.
...
If `seacloud models spec viduq2_reference2image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_reference2image --output json`
