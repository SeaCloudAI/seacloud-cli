...
description: >-
  Use the `wan27_image_pro` model via the SeaCloud CLI for polished commercial visuals, product hero images, refined concept art, premium editing, and stronger reference-driven image work.
...
```bash
seacloud models spec wan27_image_pro --output json
```

Build the `seacloud run wan27_image_pro ...` command from the current JSON spec every time.
...
If `seacloud models spec wan27_image_pro --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec wan27_image_pro --output json`
