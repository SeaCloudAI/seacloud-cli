...
name: wan2.7-image
description: >-
  Use the `wan27_image` model via the SeaCloud CLI for general image generation, fast visual iteration, image editing, and multi-image reference work.
  Make sure to use this skill for any broad creative image task that sounds exploratory, draft-stage, or speed-first, especially when the user wants strong coverage across generation, editing, and reference-driven image work without jumping to a premium final-polish model.
  Prefer this skill over heavier image models when breadth, speed, and iteration matter more than premium finish.
---

# `wan27_image` - Wan2.7 Image

**Provider:** Aliyun | **CLI Model ID:** `wan27_image` | **Inputs:** Text | Image | **Outputs:** Image
...
```bash
seacloud models spec wan27_image --output json
```

Build the `seacloud run wan27_image ...` command from the current JSON spec every time.
...
If `seacloud models spec wan27_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec wan27_image --output json`
