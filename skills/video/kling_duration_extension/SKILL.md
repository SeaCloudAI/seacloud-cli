---
name: kling_duration_extension
description: >-
  Use the `kirin_duration_extension` model via the SeaCloud CLI for extending an existing Kling clip instead of starting a new one from scratch.
  Make sure to use this skill whenever the user already has a supported Kling video and wants continuation, duration extension, or forward expansion.
  Prefer this skill over fresh video generation when the user wants to continue an existing result rather than replace it.
---

# `kirin_duration_extension` - Kling Duration Extension

**Provider:** Kling | **CLI Model ID:** `kirin_duration_extension` | **Inputs:** Text | Video | **Outputs:** Video
...
```bash
seacloud models spec kirin_duration_extension --output json
```

Build the `seacloud run kirin_duration_extension ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `watermark_info`.
...
4. For this model, verify the main fields first: `video_id`, `prompt`, `negative_prompt`, `cfg_scale`, `watermark_info`, `callback_url`.
5. Required child paths to keep intact: `video_id`.
...
7. Satisfy the model-specific precondition before the real run: requires an existing `video_id` or a valid uploaded video URL.
...
If `seacloud models spec kirin_duration_extension --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
If the failure points to missing prerequisites, gather or confirm them first: requires an existing `video_id` or a valid uploaded video URL.
...
1. `seacloud models spec kirin_duration_extension --output json`
