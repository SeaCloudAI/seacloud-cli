...
name: kling_v1_5_i2v
description: >-
  Use the `kirin_v1_5_i2v` model via the SeaCloud CLI for polished V1-era Kling image-to-video generation from a single source image.
  Make sure to use this skill whenever the user already has a source image and wants a stronger early-generation Kling result instead of the simplest baseline.
  Prefer this skill over the earliest lightweight i2v variants when finish matters more than minimal friction.
---

# `kirin_v1_5_i2v` - Kling V1.5 - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v1_5_i2v` | **Inputs:** Image | Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v1_5_i2v --output json
```

Build the `seacloud run kirin_v1_5_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v1_5_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v1_5_i2v --output json`
