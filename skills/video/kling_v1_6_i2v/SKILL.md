...
name: kling_v1_6_i2v
description: >-
  Use the `kirin_v1_6_i2v` model via the SeaCloud CLI for reliable Kling image-to-video generation with smoother motion transitions than earlier V1 variants.
  Make sure to use this skill whenever the user already has a source image and wants a solid fast baseline with stronger motion stability.
  Prefer this skill over `kirin_v1_i2v` when the user wants a better first pass without moving into heavier premium variants.
---

# `kirin_v1_6_i2v` - Kling V1.6 - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v1_6_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v1_6_i2v --output json
```

Build the `seacloud run kirin_v1_6_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v1_6_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v1_6_i2v --output json`
