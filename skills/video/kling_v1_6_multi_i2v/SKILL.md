---
name: kling_v1_6_multi_i2v
description: >-
  Use the `kirin_v1_6_multi_i2v` model via the SeaCloud CLI for multi-image Kling video generation with subject consistency across several reference images.
  Make sure to use this skill whenever the user has multiple reference images and wants them merged into one coherent video rather than animating just a single source image.
  Prefer this skill over single-image Kling video models when consistency across multiple references matters.
---

# `kirin_v1_6_multi_i2v` - Kling V1.6 Multi Image2Video

**Provider:** Kling | **CLI Model ID:** `kirin_v1_6_multi_i2v` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v1_6_multi_i2v --output json
```

Build the `seacloud run kirin_v1_6_multi_i2v ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `image_list`, `watermark_info`.
...
4. For this model, verify the main fields first: `image_list`, `prompt`, `negative_prompt`, `mode`, `duration`, `aspect_ratio`.
5. Required child paths to keep intact: `image_list`, `image_list.image`, `prompt`.
...
If `seacloud models spec kirin_v1_6_multi_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v1_6_multi_i2v --output json`
