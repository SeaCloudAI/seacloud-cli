...
name: kling_v1_6_t2v
description: >-
  Use the `kirin_v1_6_t2v` model via the SeaCloud CLI for reliable prompt-only Kling video generation with stronger motion stability than the earliest baseline.
  Make sure to use this skill whenever the user wants text-to-video without a source image and needs a solid fast baseline rather than a premium master-tier result.
  Prefer this skill over `kirin_v1_t2v` when the user wants a better first pass without moving into heavier high-end variants.
---

# `kirin_v1_6_t2v` - Kling V1.6 - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v1_6_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v1_6_t2v --output json
```

Build the `seacloud run kirin_v1_6_t2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v1_6_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v1_6_t2v --output json`
