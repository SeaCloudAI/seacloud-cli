...
name: kling_v1_i2v
description: >-
  Use the `kirin_v1_i2v` model via the SeaCloud CLI for basic Kling image-to-video generation from a single source image.
  Make sure to use this skill whenever the user wants a straightforward animation pass from an image and does not need newer refinements or premium finish.
  Prefer this skill when low-friction exploration matters more than motion quality upgrades.
---

# `kirin_v1_i2v` - Kling V1 - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v1_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v1_i2v --output json
```

Build the `seacloud run kirin_v1_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v1_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v1_i2v --output json`
