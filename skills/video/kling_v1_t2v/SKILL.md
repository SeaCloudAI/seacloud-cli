...
name: kling_v1_t2v
description: >-
  Use the `kirin_v1_t2v` model via the SeaCloud CLI for lightweight prompt-only Kling video generation and simple motion ideas.
  Make sure to use this skill whenever the user wants straightforward text-to-video output without supplying a source image and does not need newer or premium Kling quality.
  Prefer this skill when low-friction exploration matters more than later-generation refinements.
---

# `kirin_v1_t2v` - Kling V1 - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v1_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v1_t2v --output json
```

Build the `seacloud run kirin_v1_t2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v1_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v1_t2v --output json`
