...
name: kling_v2_1_i2v
description: >-
  Use the `kirin_v2_1_i2v` model via the SeaCloud CLI for refined Kling image-to-video generation with better motion control and stronger stability than older baseline variants.
  Make sure to use this skill whenever the user already has a source image and wants a balanced upgrade from the V1 line without jumping to the master tier.
  Prefer this skill when the request sounds like a mid-tier quality step-up rather than the fastest or most premium option.
---

# `kirin_v2_1_i2v` - Kling V2.1 - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_1_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_1_i2v --output json
```

Build the `seacloud run kirin_v2_1_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_1_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_1_i2v --output json`
