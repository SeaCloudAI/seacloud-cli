---
name: kling_v2_1_master_i2v
description: >-
  Use the `kirin_v2_1_master_i2v` model via the SeaCloud CLI for master-tier Kling image-to-video generation with stronger cinematic finish and premium motion quality.
  Make sure to use this skill whenever the user already has a source image and wants a polished, client-facing, or higher-end i2v result.
  Prefer this skill over lighter or older Kling i2v variants when premium finish matters more than speed.
---

# `kirin_v2_1_master_i2v` - Kling V2.1 Master - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_1_master_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_1_master_i2v --output json
```

Build the `seacloud run kirin_v2_1_master_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_1_master_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_1_master_i2v --output json`
