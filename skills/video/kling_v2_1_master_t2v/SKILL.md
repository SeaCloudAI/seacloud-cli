---
name: kling_v2_1_master_t2v
description: >-
  Use the `kirin_v2_1_master_t2v` model via the SeaCloud CLI for master-tier Kling text-to-video generation with stronger polish and premium finish.
  Make sure to use this skill whenever the user wants prompt-only video generation that feels cinematic, premium, or client-facing rather than merely fast.
  Prefer this skill over older or lighter Kling text-to-video variants when quality and finish matter more than turnaround speed.
---

# `kirin_v2_1_master_t2v` - Kling V2.1 Master - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_1_master_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_1_master_t2v --output json
```

Build the `seacloud run kirin_v2_1_master_t2v ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `multi_prompt`, `camera_control`, `watermark_info`.
...
4. For this model, verify the main fields first: `multi_shot`, `shot_type`, `prompt`, `multi_prompt`, `negative_prompt`, `sound`.
...
If `seacloud models spec kirin_v2_1_master_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_1_master_t2v --output json`
