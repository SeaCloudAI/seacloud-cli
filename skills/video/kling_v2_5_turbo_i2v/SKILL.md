...
description: >-
  Use the `kirin_v2_5_turbo_i2v` model via the SeaCloud CLI for fast Kling image-to-video generation from a single source image.
...
```bash
seacloud models spec kirin_v2_5_turbo_i2v --output json
```

Build the `seacloud run kirin_v2_5_turbo_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_5_turbo_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_5_turbo_i2v --output json`
