---
name: kling_v2_5_turbo_t2v
description: >-
  Use the `kirin_v2_5_turbo_t2v` model via the SeaCloud CLI for fast Kling text-to-video generation with expressive single-subject motion.
  Make sure to use this skill whenever the user wants quick prompt-only motion experiments, character-led effects, or rapid iteration without a source image.
  Prefer this skill over heavier Kling text-to-video variants when speed and expressive iteration matter more than maximum polish.
---

# `kirin_v2_5_turbo_t2v` - Kling V2.5 Turbo - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_5_turbo_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_5_turbo_t2v --output json
```

Build the `seacloud run kirin_v2_5_turbo_t2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_5_turbo_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_5_turbo_t2v --output json`
