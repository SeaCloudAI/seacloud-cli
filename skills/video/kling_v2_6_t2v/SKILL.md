...
name: kling_v2_6_t2v
description: >-
  Use the `kirin_v2_6_t2v` model via the SeaCloud CLI for advanced Kling text-to-video generation with smoother transitions and stronger handling of more complex scenes.
  Make sure to use this skill whenever the user wants prompt-only video generation without a source image and the prompt implies richer interaction, multi-subject motion, or more advanced scene behavior.
  Prefer this skill over older non-V3 Kling text-to-video variants when scene complexity matters more than raw speed.
---

# `kirin_v2_6_t2v` - Kling V2.6 - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_6_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_6_t2v --output json
```

Build the `seacloud run kirin_v2_6_t2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_6_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_6_t2v --output json`
