---
name: kling_v2_master_i2v
description: >-
  Use the `kirin_v2_master_i2v` model via the SeaCloud CLI for cinema-grade Kling image-to-video generation with stronger cinematography and premium finish.
  Make sure to use this skill whenever the user already has a source image and wants a polished, cinematic, or premium i2v output rather than a fast draft.
  Prefer this skill over lighter Kling i2v variants when quality matters more than turnaround.
---

# `kirin_v2_master_i2v` - Kling V2 Master - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_master_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_master_i2v --output json
```

Build the `seacloud run kirin_v2_master_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_master_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_master_i2v --output json`
