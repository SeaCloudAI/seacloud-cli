---
name: kling_v2_master_t2v
description: >-
  Use the `kirin_v2_master_t2v` model via the SeaCloud CLI for cinema-grade Kling text-to-video generation with stronger visual fidelity than baseline V1 or V2 variants.
  Make sure to use this skill whenever the user wants prompt-only video generation that should look cinematic, premium, or final-facing.
  Prefer this skill over lighter or older Kling text-to-video variants when polish matters more than iteration speed.
---

# `kirin_v2_master_t2v` - Kling V2 Master - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v2_master_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v2_master_t2v --output json
```

Build the `seacloud run kirin_v2_master_t2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v2_master_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v2_master_t2v --output json`
