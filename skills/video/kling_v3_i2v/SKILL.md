...
name: kling_v3_i2v
description: >-
  Use the `kirin_v3_i2v` model via the SeaCloud CLI for flagship Kling image-to-video generation from a single source image.
  Make sure to use this skill whenever the user wants the newest high-end Kling i2v option with stronger overall quality than older tiers.
  Prefer this skill over earlier Kling i2v variants when the request is explicitly premium or final-facing.
---

# `kirin_v3_i2v` - Kling V3 - Image to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v3_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec kirin_v3_i2v --output json
```

Build the `seacloud run kirin_v3_i2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v3_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v3_i2v --output json`
