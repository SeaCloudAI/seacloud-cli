---
name: kling_v3_omni_video
description: >-
  Use the `kirin_v3_omni_video` model via the SeaCloud CLI for flagship multimodal Kling video generation across text, image, video, and custom element inputs.
  Make sure to use this skill whenever the user wants a flexible high-end multimodal video workflow instead of a simpler prompt-only or single-image animation flow.
  Prefer this skill over older multimodal Kling video models when the request sounds premium, controlled, or final-deliverable.
---

# `kirin_v3_omni_video` - Kling V3 Omni Video

**Provider:** Kling | **CLI Model ID:** `kirin_v3_omni_video` | **Inputs:** Text | Image | Video | **Outputs:** Video
...
```bash
seacloud models spec kirin_v3_omni_video --output json
```

Build the `seacloud run kirin_v3_omni_video ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `multi_prompt`, `image_list`, `element_list`, `video_list`.
...
4. For this model, verify the main fields first: `multi_shot`, `shot_type`, `prompt`, `multi_prompt`, `image_list`, `element_list`.
5. Required child paths to keep intact: `image_list.image_url`, `element_list.element_id`, `video_list.video_url`.
...
If `seacloud models spec kirin_v3_omni_video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v3_omni_video --output json`
