...
name: kling_v3_t2v
description: >-
  Use the `kirin_v3_t2v` model via the SeaCloud CLI for flagship Kling text-to-video generation with stronger overall rendering and newer model behavior.
  Make sure to use this skill whenever the user wants the latest high-end Kling prompt-to-video option without supplying a source image.
  Prefer this skill over older Kling text-to-video variants when the user wants the newest premium text-only Kling workflow.
---

# `kirin_v3_t2v` - Kling V3 - Text to Video

**Provider:** Kling | **CLI Model ID:** `kirin_v3_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec kirin_v3_t2v --output json
```

Build the `seacloud run kirin_v3_t2v ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_v3_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_v3_t2v --output json`
