...
name: kling_video_o1
description: >-
  Use the `kirin_video_o1` model via the SeaCloud CLI for broad multimodal Kling video generation across text, image, video, and custom element inputs.
  Make sure to use this skill whenever the user wants one flexible multimodal video workflow instead of a narrower text-only or image-only path.
  Prefer this skill when multimodal breadth matters more than using the newest premium Kling multimodal line.
---

# `kirin_video_o1` - Kling Video O1

**Provider:** Kling | **CLI Model ID:** `kirin_video_o1` | **Inputs:** Text | Image | Video | **Outputs:** Video
...
```bash
seacloud models spec kirin_video_o1 --output json
```

Build the `seacloud run kirin_video_o1 ...` command from the current JSON spec every time.
...
If `seacloud models spec kirin_video_o1 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec kirin_video_o1 --output json`
