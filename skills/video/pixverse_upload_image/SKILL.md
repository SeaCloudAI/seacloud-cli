---
name: pixverse_upload_image
description: >-
  Use the `pixverse_upload_image` model via the SeaCloud CLI for the PixVerse upload step that converts a source image into a reusable `img_id` for downstream PixVerse video workflows.
  Make sure to use this skill whenever the next PixVerse call expects `img_id`, `first_frame_img`, or `last_frame_img` instead of a raw image URL.
  Prefer this as the required preflight step before PixVerse image-to-video or transition generation.
---

# `pixverse_upload_image` - Pixverse Upload Image

**Provider:** PixVerse | **CLI Model ID:** `pixverse_upload_image` | **Inputs:** Image | **Outputs:** Image
...
```bash
seacloud models spec pixverse_upload_image --output json
```

Build the `seacloud run pixverse_upload_image ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload is centered on flat top-level fields such as `image`, `image_url`.
...
4. For this model, verify the main fields first: `image`, `image_url`.
5. Required child paths to keep intact: no additional required child paths are called out in the design table, so confirm them directly from the live JSON spec.
...
If `seacloud models spec pixverse_upload_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec pixverse_upload_image --output json`
...
- For this model, keep the returned upload ID such as `img_id` or the structured asset identifier that downstream PixVerse calls require.
