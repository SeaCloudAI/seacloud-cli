---
name: pixverse_v6_i2v
description: >-
  Use the `pixverse_v6_i2v` model via the SeaCloud CLI for PixVerse image-to-video generation with cinematic camera motion, native audio, and up to 1080p output.
  Make sure to use this skill whenever the user wants a more cinematic PixVerse-style animation pipeline from a source image and is willing to use the required upload pre-step.
  Prefer this skill when native audio and cinematic motion language matter more than the simplest image-to-video workflow.
---

# `pixverse_v6_i2v` - Pixverse V6 - Image to Video

**Provider:** PixVerse | **CLI Model ID:** `pixverse_v6_i2v` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec pixverse_v6_i2v --output json
```

Build the `seacloud run pixverse_v6_i2v ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload uses flat top-level fields plus structured JSON fields such as `img_ids`.
If this skill text and the live JSON spec ever disagree, trust the live JSON spec.

## CLI Parameter Mapping

When translating the live JSON spec into `seacloud run`:
...
4. For this model, verify the main fields first: `img_id`, `img_ids`, `prompt`, `negative_prompt`, `duration`, `quality`.
5. Required child paths to keep intact: `img_id`, `prompt`, `duration`, `quality`.
...
7. Satisfy the model-specific precondition before the real run: requires a prior uploaded image or resource ID; may require multi-image reference arrays or reusable asset structures.
...
If `seacloud` is missing or unavailable, install it yourself:
...
If `seacloud models spec pixverse_v6_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
If `seacloud run` fails because of parameter shape, reopen the live JSON spec, correct the field paths, run `--dry-run`, and retry only after the dry-run shape is accepted.
If the failure points to missing prerequisites, gather or confirm them first: requires a prior uploaded image or resource ID; may require multi-image reference arrays or reusable asset structures.

## FAQ

### What if `seacloud` is not installed?

Install it yourself with `npm install -g @seacloudai/seacloud-cli`, then rerun the original `seacloud models spec ...` or `seacloud run ...` command. Do not ask the user to perform the installation for you unless the environment blocks package installation entirely.

### What if the user is not logged in?

Run `seacloud auth status` first. If the session is missing or expired, run `seacloud auth login` yourself and guide the user through the browser, URL, or device-code step that appears.
...
Because `seacloud run` does not accept invented CLI flags derived from JSON paths. Reopen the live JSON spec and translate it back into `--param key=value`, dot notation for accepted object fields, and full JSON strings for arrays or objects.
...
1. `seacloud models spec pixverse_v6_i2v --output json`
...
- `seacloud run` waits for the final result automatically.
