---
name: pixverse_v6_t2v
description: >-
  Use the `pixverse_v6_t2v` model via the SeaCloud CLI for PixVerse prompt-only video generation with cinematic motion, longer outputs, native audio, and high-resolution delivery.
  Make sure to use this skill whenever the user wants text-to-video without a source image and the request sounds like cinematic prompt-to-video, native audio, or PixVerse-style motion output.
  Prefer this skill over simpler prompt-only video models when audio and cinematic framing matter.
---

# `pixverse_v6_t2v` - Pixverse V6 - Text to Video

**Provider:** PixVerse | **CLI Model ID:** `pixverse_v6_t2v` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec pixverse_v6_t2v --output json
```

Build the `seacloud run pixverse_v6_t2v ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload is centered on flat top-level fields such as `prompt`, `negative_prompt`, `aspect_ratio`, `duration`.
...
4. For this model, verify the main fields first: `prompt`, `negative_prompt`, `aspect_ratio`, `duration`, `quality`, `motion_mode`.
5. Required child paths to keep intact: `prompt`, `aspect_ratio`, `duration`, `quality`.
...
If `seacloud models spec pixverse_v6_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec pixverse_v6_t2v --output json`
