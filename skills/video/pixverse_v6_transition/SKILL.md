---
name: pixverse_v6_transition
description: >-
  Use the `pixverse_v6_transition` model via the SeaCloud CLI for PixVerse first-frame to last-frame transition video generation with cinematic motion and native audio support.
  Make sure to use this skill whenever the user wants a bridge shot, transition clip, or first-to-last-frame transformation between two anchor images.
  Prefer this skill when the request is explicitly a transition rather than a generic image-to-video animation.
---

# `pixverse_v6_transition` - Pixverse V6 - Transition

**Provider:** PixVerse | **CLI Model ID:** `pixverse_v6_transition` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec pixverse_v6_transition --output json
```

Build the `seacloud run pixverse_v6_transition ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload is centered on flat top-level fields such as `prompt`, `first_frame_img`, `last_frame_img`, `duration`.
...
4. For this model, verify the main fields first: `prompt`, `first_frame_img`, `last_frame_img`, `duration`, `quality`, `motion_mode`.
5. Required child paths to keep intact: `prompt`, `first_frame_img`, `last_frame_img`, `duration`, `quality`.
...
7. Satisfy the model-specific precondition before the real run: requires a prior uploaded image or resource ID.
...
If `seacloud models spec pixverse_v6_transition --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
If the failure points to missing prerequisites, gather or confirm them first: requires a prior uploaded image or resource ID.
...
1. `seacloud models spec pixverse_v6_transition --output json`
