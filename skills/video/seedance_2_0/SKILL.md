...
name: seedance_2_0
description: >-
  Use the `spark_dance_v2_0` model via the SeaCloud CLI for cinematic multimodal video generation across text, image, video, and audio inputs.
  Make sure to use this skill whenever the user wants Seedance 2.0 for creative storytelling, ads, short-form video, or multimodal video editing, even if they do not mention the exact model ID.
  Prefer this skill over the Fast variant when motion quality, richer control, or more polished output matters more than turnaround speed.
---

# `spark_dance_v2_0` - Seedance 2.0

**Provider:** Volces | **CLI Model ID:** `spark_dance_v2_0` | **Inputs:** Text | Image | Video | Audio | **Outputs:** Video
...
```bash
seacloud models spec spark_dance_v2_0 --output json
```

Build the `seacloud run spark_dance_v2_0 ...` command from the current JSON spec every time.
...
If `seacloud models spec spark_dance_v2_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec spark_dance_v2_0 --output json`
