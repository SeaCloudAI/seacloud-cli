...
description: >-
  Use the `spark_dance_v2_0_fast` model via the SeaCloud CLI for faster multimodal video generation across text, image, video, and audio inputs.
...
```bash
seacloud models spec spark_dance_v2_0_fast --output json
```

Build the `seacloud run spark_dance_v2_0_fast ...` command from the current JSON spec every time.
...
If `seacloud models spec spark_dance_v2_0_fast --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec spark_dance_v2_0_fast --output json`
