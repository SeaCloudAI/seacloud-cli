---
name: vidu20_img2video
description: >-
  Use the `vidu20_img2video` model via the SeaCloud CLI for classic Vidu image-to-video generation with improved motion and better detail retention than older entry models.
  Make sure to use this skill whenever the user already has a source image and wants a solid Vidu i2v baseline rather than the newest premium tier.
  Prefer this skill when the user wants a proven middle-ground option in the older Vidu line.
---

# `vidu20_img2video` - Vidu 2.0 - Image to Video

**Provider:** Vidu | **CLI Model ID:** `vidu20_img2video` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec vidu20_img2video --output json
```

Build the `seacloud run vidu20_img2video ...` command from the current JSON spec every time.
...
If `seacloud models spec vidu20_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec vidu20_img2video --output json`
