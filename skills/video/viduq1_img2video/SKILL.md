---
name: viduq1_img2video
description: >-
  Use the `viduq1_img2video` model via the SeaCloud CLI for cost-conscious or rapid-prototype image-to-video generation in the Vidu family.
  Make sure to use this skill whenever the user already has a source image and values speed or budget more than premium fidelity.
  Prefer this skill over newer Vidu tiers when quick prototyping matters more than finish.
---

# `viduq1_img2video` - Vidu Q1 - Image to Video

**Provider:** Vidu | **CLI Model ID:** `viduq1_img2video` | **Inputs:** Image | **Outputs:** Video
...
```bash
seacloud models spec viduq1_img2video --output json
```

Build the `seacloud run viduq1_img2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq1_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq1_img2video --output json`
