---
name: viduq2_pro_extend
description: >-
  Use the `viduq2_pro_extend` model via the SeaCloud CLI for premium Vidu video extension and continuation generation.
  Make sure to use this skill whenever the user wants an existing Vidu clip to continue with stronger continuity and more polished output instead of starting over.
  Prefer this skill over faster extension variants when finish and control matter more than speed.
---

# `viduq2_pro_extend` - Vidu Q2 Pro - Video Extension

**Provider:** Vidu | **CLI Model ID:** `viduq2_pro_extend` | **Inputs:** Video | **Outputs:** Video
...
```bash
seacloud models spec viduq2_pro_extend --output json
```

Build the `seacloud run viduq2_pro_extend ...` command from the current JSON spec every time.
...
4. For this model, verify the main fields first: `video_creation_id`, `video_url`, `images`, `prompt`, `resolution`, `duration`.
...
If `seacloud models spec viduq2_pro_extend --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_pro_extend --output json`
