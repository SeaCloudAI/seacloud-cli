---
name: viduq2_pro_fast_img2video
description: >-
  Use the `viduq2_pro_fast_img2video` model via the SeaCloud CLI for fast Vidu image-to-video generation with stronger value and better quality than the lightest entry tier.
  Make sure to use this skill whenever the user already has a source image and wants a quicker premium-leaning option without stepping all the way up to the top-end line.
  Prefer this skill over cheaper or older Vidu i2v variants when output quality matters but speed still matters too.
---

# `viduq2_pro_fast_img2video` - Vidu Q2 Pro Fast - Image to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_pro_fast_img2video` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec viduq2_pro_fast_img2video --output json
```

Build the `seacloud run viduq2_pro_fast_img2video ...` command from the current JSON spec every time.
...
4. For this model, verify the main fields first: `images`, `prompt`, `audio`, `audio_type`, `voice_id`, `is_rec`.
...
If `seacloud models spec viduq2_pro_fast_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_pro_fast_img2video --output json`
