---
name: viduq2_pro_fast_start_end2video
description: >-
  Use the `viduq2_pro_fast_start_end2video` model via the SeaCloud CLI for faster start-frame and end-frame guided Vidu video generation.
  Make sure to use this skill whenever the user wants a guided bridge shot between two anchor images but still values speed and efficiency.
  Prefer this skill over the cheaper or older start-end variants when the user wants a stronger result without going fully flagship.
---

# `viduq2_pro_fast_start_end2video` - Vidu Q2 Pro Fast - Start end to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_pro_fast_start_end2video` | **Inputs:** Image | Text | **Outputs:** Video
...
```bash
seacloud models spec viduq2_pro_fast_start_end2video --output json
```

Build the `seacloud run viduq2_pro_fast_start_end2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq2_pro_fast_start_end2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_pro_fast_start_end2video --output json`
