---
name: viduq2_pro_fast_text2video
description: >-
  Use the `viduq2_pro_fast_text2video` model via the SeaCloud CLI for fast Vidu text-to-video generation with stronger value and better quality than the lightest entry tier.
  Make sure to use this skill whenever the user wants prompt-only video generation without a source image and needs a quicker premium-leaning option rather than a flagship one.
  Prefer this skill over cheaper or older Vidu text-to-video variants when the user wants better output without paying for the highest-end line.
---

# `viduq2_pro_fast_text2video` - Vidu Q2 Pro Fast - Text to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_pro_fast_text2video` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec viduq2_pro_fast_text2video --output json
```

Build the `seacloud run viduq2_pro_fast_text2video ...` command from the current JSON spec every time.
Do not trust cached examples, old field names, stale screenshots, or historical command fragments.
The current payload is centered on flat top-level fields such as `style`, `prompt`, `duration`, `seed`.
...
4. For this model, verify the main fields first: `style`, `prompt`, `duration`, `seed`, `aspect_ratio`, `resolution`.
...
If `seacloud models spec viduq2_pro_fast_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_pro_fast_text2video --output json`
