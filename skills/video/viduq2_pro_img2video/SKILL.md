---
name: viduq2_pro_img2video
description: >-
  Use the `viduq2_pro_img2video` model via the SeaCloud CLI for premium Vidu image-to-video generation with stronger motion quality and 1080p output.
  Make sure to use this skill whenever the user already has a source image and wants a clear quality step up from the balanced or turbo lines.
  Prefer this skill when the request sounds polished, premium, or closer to final delivery.
---

# `viduq2_pro_img2video` - Vidu Q2 Pro - Image to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_pro_img2video` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec viduq2_pro_img2video --output json
```

Build the `seacloud run viduq2_pro_img2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq2_pro_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_pro_img2video --output json`
