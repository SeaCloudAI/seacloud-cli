---
name: viduq2_text2video
description: >-
  Use the `viduq2_text2video` model via the SeaCloud CLI for balanced Vidu text-to-video generation with a strong quality-speed tradeoff and broad resolution support.
  Make sure to use this skill whenever the user wants prompt-only video generation without a source image and needs a middle-ground Vidu option instead of the cheapest or the flagship tier.
  Prefer this skill when the request sounds balanced rather than explicitly fast-only or premium-only.
---

# `viduq2_text2video` - Vidu Q2 - Text to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_text2video` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec viduq2_text2video --output json
```

Build the `seacloud run viduq2_text2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq2_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_text2video --output json`
