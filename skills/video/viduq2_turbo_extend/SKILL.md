---
name: viduq2_turbo_extend
description: >-
  Use the `viduq2_turbo_extend` model via the SeaCloud CLI for faster Vidu video extension and continuation generation.
  Make sure to use this skill whenever the user wants to continue an existing clip but values quicker turnaround more than premium polish.
  Prefer this skill over heavier extension variants when speed matters more than finish.
---

# `viduq2_turbo_extend` - Vidu Q2 Turbo - Video Extension

**Provider:** Vidu | **CLI Model ID:** `viduq2_turbo_extend` | **Inputs:** Video | **Outputs:** Video
...
```bash
seacloud models spec viduq2_turbo_extend --output json
```

Build the `seacloud run viduq2_turbo_extend ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq2_turbo_extend --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_turbo_extend --output json`
