---
name: viduq2_turbo_img2video
description: >-
  Use the `viduq2_turbo_img2video` model via the SeaCloud CLI for fast Vidu image-to-video generation with a strong quality-speed tradeoff.
  Make sure to use this skill whenever the user already has a source image and wants quicker turnaround than the Pro line without dropping to the oldest or cheapest tier.
  Prefer this skill when speed matters more than premium finish but the user still wants a capable modern baseline.
---

# `viduq2_turbo_img2video` - Vidu Q2 Turbo - Image to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_turbo_img2video` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec viduq2_turbo_img2video --output json
```

Build the `seacloud run viduq2_turbo_img2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq2_turbo_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_turbo_img2video --output json`
