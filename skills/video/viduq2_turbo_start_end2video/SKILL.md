---
name: viduq2_turbo_start_end2video
description: >-
  Use the `viduq2_turbo_start_end2video` model via the SeaCloud CLI for fast start-frame and end-frame guided Vidu video generation.
  Make sure to use this skill whenever the user wants a first-to-last-frame transition or bridge shot but values quick iteration more than premium finish.
  Prefer this skill over heavier start-end variants when speed matters more than polish.
---

# `viduq2_turbo_start_end2video` - Vidu Q2 Turbo - Start end to Video

**Provider:** Vidu | **CLI Model ID:** `viduq2_turbo_start_end2video` | **Inputs:** Image | Text | **Outputs:** Video
...
```bash
seacloud models spec viduq2_turbo_start_end2video --output json
```

Build the `seacloud run viduq2_turbo_start_end2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq2_turbo_start_end2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq2_turbo_start_end2video --output json`
