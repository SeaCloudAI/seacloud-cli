---
name: viduq3_pro_img2video
description: >-
  Use the `viduq3_pro_img2video` model via the SeaCloud CLI for flagship Vidu image-to-video generation with audio-visual output and high-end detail.
  Make sure to use this skill whenever the user already has a source image and wants premium or client-facing animation rather than a fast draft.
  Prefer this skill over lower Vidu i2v tiers when finish, detail, and delivery quality matter more than speed.
---

# `viduq3_pro_img2video` - Vidu Q3 Pro - Image to Video

**Provider:** Vidu | **CLI Model ID:** `viduq3_pro_img2video` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec viduq3_pro_img2video --output json
```

Build the `seacloud run viduq3_pro_img2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq3_pro_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq3_pro_img2video --output json`
