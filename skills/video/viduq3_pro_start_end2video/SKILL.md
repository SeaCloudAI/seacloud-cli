---
name: viduq3_pro_start_end2video
description: >-
  Use the `viduq3_pro_start_end2video` model via the SeaCloud CLI for flagship start-frame and end-frame guided Vidu video generation with audio-visual output and premium finish.
  Make sure to use this skill whenever the user wants the highest-end bridge shot or first-to-last-frame transition workflow in the Vidu family.
  Prefer this skill over older or faster start-end variants when final quality matters more than speed.
---

# `viduq3_pro_start_end2video` - Vidu Q3 Pro - Start End to Video

**Provider:** Vidu | **CLI Model ID:** `viduq3_pro_start_end2video` | **Inputs:** Image | Text | **Outputs:** Video
...
```bash
seacloud models spec viduq3_pro_start_end2video --output json
```

Build the `seacloud run viduq3_pro_start_end2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq3_pro_start_end2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq3_pro_start_end2video --output json`
