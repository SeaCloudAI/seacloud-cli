---
name: viduq3_pro_text2video
description: >-
  Use the `viduq3_pro_text2video` model via the SeaCloud CLI for flagship Vidu text-to-video generation with audio-visual output, stronger detail, and premium final quality.
  Make sure to use this skill whenever the user wants prompt-only video generation without a source image and the request sounds cinematic, polished, client-facing, or final-deliverable.
  Prefer this skill over lower Vidu text-to-video tiers when finish and premium output matter more than speed.
---

# `viduq3_pro_text2video` - Vidu Q3 Pro - Text to Video

**Provider:** Vidu | **CLI Model ID:** `viduq3_pro_text2video` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec viduq3_pro_text2video --output json
```

Build the `seacloud run viduq3_pro_text2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq3_pro_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq3_pro_text2video --output json`
