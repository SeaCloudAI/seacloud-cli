---
name: viduq3_turbo_img2video
description: >-
  Use the `viduq3_turbo_img2video` model via the SeaCloud CLI for faster Vidu Q3 image-to-video generation with audio-visual output and newer generation quality.
  Make sure to use this skill whenever the user already has a source image and wants Q3-level capability but values speed more than Pro-level finish.
  Prefer this skill over `viduq3_pro_img2video` when turnaround matters more than maximum polish.
---

# `viduq3_turbo_img2video` - Vidu Q3 Turbo - Image to Video

**Provider:** Vidu | **CLI Model ID:** `viduq3_turbo_img2video` | **Inputs:** Text | Image | **Outputs:** Video
...
```bash
seacloud models spec viduq3_turbo_img2video --output json
```

Build the `seacloud run viduq3_turbo_img2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq3_turbo_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq3_turbo_img2video --output json`
