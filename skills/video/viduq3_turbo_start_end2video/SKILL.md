...
description: >-
  Use the `viduq3_turbo_start_end2video` model via the SeaCloud CLI for faster Q3-era start-frame and end-frame guided Vidu video generation.
...
```bash
seacloud models spec viduq3_turbo_start_end2video --output json
```

Build the `seacloud run viduq3_turbo_start_end2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq3_turbo_start_end2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq3_turbo_start_end2video --output json`
