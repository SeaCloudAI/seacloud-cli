---
name: viduq3_turbo_text2video
description: >-
  Use the `viduq3_turbo_text2video` model via the SeaCloud CLI for faster Vidu Q3 text-to-video generation with audio-visual output and newer generation quality.
  Make sure to use this skill whenever the user wants prompt-only video generation without a source image and prefers Q3-era capabilities with better speed than the Pro line.
  Prefer this skill over `viduq3_pro_text2video` when turnaround speed matters more than maximum finish.
---

# `viduq3_turbo_text2video` - Vidu Q3 Turbo - Text to Video

**Provider:** Vidu | **CLI Model ID:** `viduq3_turbo_text2video` | **Inputs:** Text | **Outputs:** Video
...
```bash
seacloud models spec viduq3_turbo_text2video --output json
```

Build the `seacloud run viduq3_turbo_text2video ...` command from the current JSON spec every time.
...
If `seacloud models spec viduq3_turbo_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `seacloud models spec viduq3_turbo_text2video --output json`
